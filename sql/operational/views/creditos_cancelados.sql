GO
CREATE OR ALTER VIEW gc_creditos_castigados WITH ENCRYPTION AS
--- ############
--- NOTAS
--- ############
...
-- Permite calcular el saldo de cancelación
--- ------------
...
WHERE
------
    FORMAT(FECHA_MOV, 'yyyyMM') = FORMAT(GETDATE(), 'yyyyMM')
--------
GROUP BY
--------
    FECHA_MOV,
...
------
    ...
    INNER JOIN CTE_AUX                  NEXO_AUX ON NEXO_AUX.CUENTA = NEXO_PRE.CUENTA AND NEXO_AUX.OTORGA = NEXO_PRE.OTORGA AND NEXO_AUX.PAGARE = NEXO_PRE.PAGARE AND NEXO_AUX.Fecha_Movimiento = NEXO_PRE.FECHA_ULT_PAGO
------
WHERE
------
...
SELECT
    ...
    Diferencia_Dias,
    CASE
        WHEN T.Tipo_Cancelacion = 'Cancelado con Anticipación' THEN ' ' + CAST(T.Cuota_Cancelacion AS varchar(10)) + '/' + CAST(T.Nro_Cuotas AS VARCHAR(10))
        ELSE '---'
