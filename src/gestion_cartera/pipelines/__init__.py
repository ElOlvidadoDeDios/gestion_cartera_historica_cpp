...
def pipeline_dim_asesor() -> None:
    ...
    loader_factory.strategy = LoaderStrategicInitial
    loader_factory.run(df=df_transformed, table=ConfigManager.table.dim.asesor)
...
def pipeline_fct_stock() -> None:
    ...
    loader_factory.run(df=df, table=ConfigManager.table.fct.stock)
...
def pipeline_fct_flow() -> None:
    ...
    loader_factory = LoaderFactory.get_loader('strategic')
    loader_factory.strategy = LoaderStrategicVariational
    loader_factory.run(df=df, table=ConfigManager.table.fct.flow)
